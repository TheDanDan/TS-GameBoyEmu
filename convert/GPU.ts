...
const GPU = {
    ...
    _tilemap: [] as number[][][],
    _objdata: [] as NumDict[],
    _objdatasorted: [] as NumDict[],
    _palette: { 'bg': [] as NumArray, 'obj0': [] as NumArray, 'obj1': [] as NumArray },
    _scanrow: [] as NumArray,
    ...
    _canvas: new CanvasRenderingContext2D() as CanvasRenderingContext2D | null,
    _scrn: new ImageData(160, 144) as any,
    ...
    reset: function () {
        ...
        for (let i = 0; i < 512; i++) {
            GPU._tilemap[i] = [] as number[][];
            for (let j = 0; j < 8; j++) {
                GPU._tilemap[i][j] = [] as number[];
                for (let k = 0; k < 8; k++) {
        ...
        console.log('GPU', 'Initialising screen.');
        var c = document.getElementById('screen') as HTMLCanvasElement;
        if (c && c.getContext) {
            ...
            if (!GPU._canvas) {
                ...
                for (let i = 0; i < GPU._scrn.data.length; i++)
        ...
        GPU._objsize = 0;
        for (let i = 0; i < 160; i++) GPU._scanrow[i] = 0;
    ...
    updatetile: function (addr: number, val: number) {
        var saddr = addr;
    ...
    updateoam: function (addr: number, val: number) {
        addr -= 0xFE00;
        var obj = addr >> 2;
        if (obj < 40) {
            switch (addr & 3) {
